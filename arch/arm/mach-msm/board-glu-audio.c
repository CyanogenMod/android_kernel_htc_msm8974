...
static int glu_get_hw_component(void)
{
	return (HTC_AUDIO_RT5501 | HTC_AUDIO_TFA9887 | HTC_AUDIO_TFA9887L);
}

static int glu_enable_digital_mic(void)
{
	return atomic_read(&dev_use_dmic);
}

int glu_enable_24b_audio(void)
{
	return 1;
}

void glu_set_q6_effect_mode(int mode)
{
	pr_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

int glu_get_q6_effect_mode(void)
{
...
static struct acoustic_ops acoustic = {
        .enable_digital_mic = glu_enable_digital_mic,
        .get_hw_component = glu_get_hw_component,
	.set_q6_effect = glu_set_q6_effect_mode,
	.get_q6_effect = glu_get_q6_effect_mode,
	.enable_24b_audio = glu_enable_24b_audio
};
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if(htc_wcd_config.init) {
		gpio_tlmm_config(GPIO_CFG(htc_wcd_config.gpio[0].gpio_no, \
			0,GPIO_CFG_OUTPUT,GPIO_CFG_PULL_DOWN,GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	}
