...
static int __set_src_div(struct mux_div_clk *md, struct clk *parent, u32 div)
{
	...
	/* Calling this procedure whith the clock disabled should never happen */
...
err_set_src_div:
	/* Ensure that warnings are printed out on error cases before attempting to repair the error */
	WARN(rc, "disabling %s\n", new_parent->dbg_name);
