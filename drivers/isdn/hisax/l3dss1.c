...
static unsigned char new_invoke_id(struct PStack *p)
{
	...
	retval = p->prot.dss1.last_invoke_id + 1; 
...
{ u_char id;
	...
	{ case DSS1_CMD_INVOKE:
			if (ic->parm.dss1_io.datalen < 0) return (-2); 

			for (proc_len = 1, i = ic->parm.dss1_io.proc >> 8; i; i++)
				i = i >> 8; 
			l = ic->parm.dss1_io.datalen + proc_len + 8; 
			if (l > 255)
				return (-2); 
			...
			*p++ = proc_len; 
			...
			memcpy(p, ic->parm.dss1_io.data, ic->parm.dss1_io.datalen); 
