...
#define CF_WRITE_DATA			BIT_0
	...
	uint32_t fcp_cmnd_dseg_address[2];	

	uint32_t fcp_rsp_dseg_address[2];	
	...
	uint32_t fcp_data_dseg_address[2];	
...
struct cmd_type_crc_2 {
	...
	uint32_t crc_context_address[2];	
...
#define ECF_INCL_FRAME_HDR	BIT_11
	...
	uint32_t tx_address[2];		
	uint32_t tx_len;		
	uint32_t rx_address[2];		
	uint32_t rx_len;		
};
