 ...
 */
#define pr_fmt(fmt) "[CAM]rumbas %s:%d " fmt, __func__, __LINE__
	...
	struct msm_actuator_set_info_t *set_info)
{
	...
	if((set_info->af_tuning_params.total_steps + 1) == (sizeof(kernel_step_table) / sizeof(int))) {
		for(; step_index <= step_boundary; step_index++) {
			a_ctrl->step_position_table[step_index] = kernel_step_table[step_index];
			CDBG("%d;", a_ctrl->step_position_table[step_index]);
		}
	} else {
	...
	int16_t code_boundary)
{
	...
	CDBG("curr_lens_pos %d, code_boundary %d\n", curr_lens_pos, code_boundary);
